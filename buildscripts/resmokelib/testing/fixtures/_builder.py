...
class ReplSetBuilder(FixtureBuilder):
    ...
    def build_fixture(self, logger, job_num, fixturelib, *args, **kwargs):  # pylint: disable=too-many-locals
        ...
        if mixed_bin_versions is None:
            ...
            if USE_LEGACY_MULTIVERSION:
                executables = [
                    latest_mongod if (x == "new") else multiversion.LAST_LTS_MONGOD_BINARY
...
def load_version(version_path_suffix=None, shell_path=None):
    ...
    with RETRIEVE_LOCK, registry.suffix(version_path_suffix):
        ...
        # latest_version = _VERSIONS["APIVersion"].FIXTURE_API_VERSION
        # if not _VERSIONS["APIVersion_last_lts"].check_api_version(latest_version):
        #    last_lts_version = _VERSIONS["APIVersion_last_lts"].FIXTURE_API_VERSION
        #    msg = f"The last_lts API version {last_lts_version} is not compatible with the latest {latest_version}."
        #    raise errors.ServerFailure(msg)
