...
public:
    ...
    void cacheIndexInfo(OperationContext* txn,
        ...
        if (_full) {
            ...
            std::unique_ptr<SortedDataInterface::Cursor> cursor =
                iam->newCursor(txn, key.firstElement().number() >= 0);
    ...
    void validateIndexes(OperationContext* txn,
        ...
        // Verifies that each multi-key index indexes at least one document with multiple keys.
        std::map<string, bool> multiKeyIndexesValid;
        ...
        if (_full) {
            while (auto document = documentCursor->next()) {
                ...
                while (i.more()) {
                    ...
                    string ins = descriptor->indexNamespace();
                    invariant(iam);

                    ValidateResults& results = indexDetailNsResultsMap[ins];
                    if (!results.valid) {
                    ...
                    if (documentKeySet.size() > 1) {
                        if (!descriptor->isMultikey(txn)) {
                            string msg = str::stream()
                                ...
                                << " is not multikey but has more than one key in document "
                                << document->data.toBson();
                            results.errors.push_back(msg);
                            results.valid = false;
                        } else if (descriptor->isMultikey(txn)) {
                            multiKeyIndexesValid[descriptor->indexNamespace()] = true;
                        }
                    }

                    for (const auto& key : documentKeySet) {
                        if (key.objsize() >= IndexKeyMaxSize) {
                            // Index keys longer than 1024 bytes are not indexed.
                            _longKeys[ins]++;
                            continue;
                        }
        ...
        if (allIndexesValid) {
            ...
            while (i.more()) {
                ...
                if (descriptor->isMultikey(txn) &&
                    !multiKeyIndexesValid[descriptor->indexNamespace()]) {
                    // A multi-key indexes is not automatically down-graded to a regular one when
                    // it no longer indexes more than one key in any document.
                    string msg = str::stream()
                        << "Index " << descriptor->indexName()
                        << " is marked multi-key, but no longer indexes multi-key documents";
        ...
        if (!allIndexesValid) {
            string msg = "Index entry(s) in one or more indexes don't point to valid documents";
...
private:
    ...
    void validateIndexKeyCount(OperationContext* txn,
        ...
        string ins = idx->indexNamespace();
        long long numIdxKeys = _keyCounts[ins];
        long long numLongKeys = _longKeys[ins];
        auto totalKeys = numLongKeys + numIdxKeys;
        ...
        if (idx->isIdIndex() && totalKeys != numRecs) {
            ...
            string msg = str::stream() << "number of _id index entries (" << numIdxKeys
                                       ...
                                       << ")";
            if (!failIndexKeyTooLong && !_full && (numIdxKeys < numRecs)) {
                results->warnings.push_back(msg);
            } else {
                results->errors.push_back(msg);
                results->valid = false;
